...
public class LongRangeAttack : MonoBehaviour
{
    ...
    public void SummonMeteor(float attackDelay, Vector3 unitPos, Vector3 oppositeUnitPos)
    {
        StartCoroutine(C_MeteorCoroutine(attackDelay, unitPos, oppositeUnitPos));
        
    }

    IEnumerator C_MeteorCoroutine(float attackDelay, Vector3 unitPos, Vector3 oppositeUnitPos)
    {

        GameObject meteor = PoolManager.Instance.Get(ProjectileType.Meteor.ToString());
        meteor.transform.position = unitPos + Vector3.up * _meteorPosY;

        SoundManager.Instance.SummonMeteor();

        Vector3 directionOfMeteor = (oppositeUnitPos - meteor.transform.position).normalized;
        float angle = Mathf.Atan2(directionOfMeteor.y, directionOfMeteor.x) * Mathf.Rad2Deg;

        meteor.transform.rotation = Quaternion.Euler(0, 0, angle);

        while (meteor.transform.position.y >= oppositeUnitPos.y+1.5)
        {
            meteor.transform.position += directionOfMeteor * Time.deltaTime * (_meteorPosY / (attackDelay/2));
